...
1. **wake** (0.108) — 가장 큰 gap. 근접부 x/D=1.06 재순환 U(-0.39, 역류), 응력 진폭. von-Kármán 비정상.
2. **jet 원거리** x/D=4 v_rms(0.62)/u_rms(0.74) — 잔여 진폭 결손.
3. **pipe 축방향** u_rms(0.66) — heavy-tail 잔여.

다음: 이 세 약점 채널을 ≥10 에이전트 병렬로 event-only 개선 탐색 → 검증 후 적용. 이후 보고서 빌드·푸시.
