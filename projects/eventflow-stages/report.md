...
| Stage 2 (한 모듈) | events, tmin4, spatial32 | paths, gate14, fd1, spatial8 | paths, gate4, fd4, 경로id 짝 |
...
| wake | **0.626** | 정직 production (타임스탬프·MAD·px-정합 bin까지). 앙상블/dual-stream(→0.655)은 지역-컷이 R²-선택으로 판명되어 **정직성 감사로 실험 변형 강등** — 앙상블 추정기 자체와 프로파일 형태(쌍봉/단봉) 발견은 유효, event-only 적용-게이트가 미해결 과제 |
...
```
repo donggeonbae/event-flow-turbulence, branch work/wake-operator-recovery-20260624 @ (정직성 철회 커밋) — 앙상블은 실험 변형
